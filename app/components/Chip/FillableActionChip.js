...
export default FillableActionChip = ({text, index, onPress, chipParentStyle={}, chipStyle={}, chipTextStyle={}, filled=false, testId={}}) => {
  return (
    <View style={[styles.ChipParent, chipParentStyle]}>
      <TouchableOpacity onPress={() => onPress(index)} style={[styles.Chip, chipStyle, filled? {backgroundColor: colorresource.liteblue} : {backgroundColor: 'white'}]} {...testId}>
          <Text style={[styles.ChipText, chipTextStyle, filled? {color: 'white'} : {}]}>{text}</Text>
      </TouchableOpacity>
    </View>
  );
}
